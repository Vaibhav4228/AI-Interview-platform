...
export const FormMockInterview = ({ initialData }: FormMockInterviewProps) => {
  ...
  const cleanAiResponse = (responseText: string) => {
    ...
    try {
      ...
      if (jsonArrayMatch) {
        try {
          return tryParse(jsonArrayMatch[0]);
        } catch {
          throw new Error("Invalid JSON format in AI response: could not parse extracted array");
        }
      }
      throw new Error(`Invalid JSON format in AI response: ${firstError instanceof Error ? firstError.message : String(firstError)}`);
    }
  };
